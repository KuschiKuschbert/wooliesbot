...
def run_checks(page: Page, results: Results, shot_dir: Path, base_url: str) -> None:
    ...
    # ------------------------------------------------------------------
    # 17 — Cross-device cloud cart sync (mock /shopping_list)
    # ------------------------------------------------------------------
    def check_cross_device_cart_sync():
        base_url_js = json.dumps(base_url.rstrip("/"))
        cloud_state = {
        ...
        shopping_list_requests = 0
        last_post_payload = {}

        def route_handler(route):
            nonlocal cloud_state
            nonlocal shopping_list_requests
            nonlocal last_post_payload
            req = route.request
            if "/shopping_list" not in req.url:
                return route.continue_()
            shopping_list_requests += 1
            if req.method == "GET":
                return route.fulfill(
                    status=200,
                    headers={"Content-Type": "application/json"},
                    body=json.dumps({"status": "success", "payload": cloud_state}),
                )
            if req.method == "POST":
                try:
                    body_raw = req.post_data
                ...
                last_post_payload = payload
                # Mock server merge: union by item key, qty=max, picked OR, latest metadata.
                def key_of(item):
                ...
                remote_items = cloud_state.get("items") or []
                incoming_items = payload.get("items") if isinstance(payload.get("items"), list) else []
                remote_tombs = cloud_state.get("tombstones") if isinstance(cloud_state.get("tombstones"), dict) else {}
                incoming_tombs = payload.get("tombstones") if isinstance(payload.get("tombstones"), dict) else {}
                tombs = dict(remote_tombs)
                for k, v in incoming_tombs.items():
                    if not k:
                        continue
                    if ms(v) >= ms(tombs.get(k)):
                        tombs[k] = v
                merged = {}
                for it in remote_items + incoming_items:
                    k = key_of(it)
                    if not k:
                        continue
                    prev = merged.get(k)
                    if not prev:
                        merged[k] = dict(it)
                        continue
                    prev_ms = ms(prev.get("updated_at"))
                    cur_ms = ms(it.get("updated_at"))
                    newer = dict(it if cur_ms >= prev_ms else prev)
                    older = prev if cur_ms >= prev_ms else it
                    newer["qty"] = max(int(prev.get("qty") or 1), int(it.get("qty") or 1), 1)
                    newer["picked"] = bool(prev.get("picked")) or bool(it.get("picked"))
                    if not newer.get("item_id"):
                        newer["item_id"] = older.get("item_id")
                    if not newer.get("name"):
                        newer["name"] = older.get("name")
                    merged[k] = newer
                items = []
                for k, it in merged.items():
                    if ms(tombs.get(k)) >= ms(it.get("updated_at")):
                        continue
                    items.append(it)
                cloud_state = {
                    "updated_at": payload.get("updated_at") or cloud_state.get("updated_at"),
                    "device_id": payload.get("device_id") or "mock",
                    "items": items,
                    "tombstones": tombs,
                }
                return route.fulfill(
        ...
        ctx = page.context
        ctx.route("**/shopping_list**", route_handler)
        page_b = ctx.new_page()

        try:
            # Device A starts with one item and pushes.
            page.evaluate(
                f"""({{ base }}) => {{
                    localStorage.setItem('write_api_url', base);
                    localStorage.setItem('write_api_secret', 'e2e');
                    localStorage.setItem('shoppingList', JSON.stringify([
                        {{ item_id: 'milk', name: 'E2E Milk', qty: 1, price: 3.5, store: 'woolworths', picked: false, updated_at: '2026-01-01T01:00:00.000Z' }}
                    ]));
                    localStorage.setItem('shoppingListTombstones', '{{}}');
                    localStorage.setItem('shoppingListLastRemoteUpdatedAt', '');
                    localStorage.removeItem('shopping_device_id');
                }}""",
                {"base": f"{base_url}/mock"},
            )
            page.reload(wait_until="domcontentloaded")
            _wait_for_app_ready(page)
            page.evaluate(
                """() => {
                    ...
                    if (typeof _shoppingListTombstones !== 'undefined') _shoppingListTombstones = {};
                    persistShoppingList();
                }"""
            )
            diag = page.evaluate(
                """() => ({
                    writeUrl: (typeof _writeApiUrl !== 'undefined') ? _writeApiUrl : 'missing',
                    stockBase: (typeof getStockWriteBase === 'function') ? getStockWriteBase() : 'no-fn',
                    syncFn: typeof syncShoppingListCloud,
                    localItems: (typeof _shoppingList !== 'undefined' && Array.isArray(_shoppingList)) ? _shoppingList.length : -1,
                })"""
            )
            page.wait_for_timeout(900)
            page.evaluate(
                """async () => {
                    ...
                    await syncShoppingListCloud('push');
                    return true;
                }"""
            )
            page.wait_for_timeout(700)
            if len(cloud_state.get("items") or []) != 1:
                return "FAIL", f"device A did not push initial cart item (diag={diag!r}, reqs={shopping_list_requests}, payload={last_post_payload!r}, cloud={cloud_state!r})"

            # Device B pulls and mutates same item with higher qty + picked.
            page_b.goto(base_url, wait_until="domcontentloaded")
            _wait_for_app_ready(page_b)
            page_b.evaluate(
                f"""({{ base }}) => {{
                    localStorage.setItem('write_api_url', base);
                    localStorage.setItem('write_api_secret', 'e2e');
            ...
            page_b.evaluate(
                """async () => {
                    await syncShoppingListCloud('poll');
                    return true;
                }"""
            )
            page_b.wait_for_function("() => (JSON.parse(localStorage.getItem('shoppingList') || '[]').length === 1)", timeout=5000)
            page_b.evaluate(
                """() => {
                    const rows = JSON.parse(localStorage.getItem('shoppingList') || '[]');
                    rows[0].qty = 2;
                    rows[0].picked = true;
                    rows[0].updated_at = '2026-01-01T02:00:00.000Z';
                    localStorage.setItem('shoppingList', JSON.stringify(rows));
                    if (typeof _shoppingList !== 'undefined') _shoppingList = rows;
                }"""
            )
            page_b.evaluate(
                """async () => {
            ...
            # Device A changes metadata later and pushes; merged cloud should keep qty=2 and picked=true.
            page.evaluate(
                """() => {
                    const rows = JSON.parse(localStorage.getItem('shoppingList') || '[]');
                    rows[0].price = 3.1;
                    rows[0].updated_at = '2026-01-01T03:00:00.000Z';
                    localStorage.setItem('shoppingList', JSON.stringify(rows));
                    if (typeof _shoppingList !== 'undefined') _shoppingList = rows;
                }"""
            )
            ...
            if not merged_ok:
                return "FAIL", "merged result did not preserve qty max + picked OR rules"
            return "PASS", "cross-device cart sync pull/push merge works"
        finally:
            try:
                page_b.close()
            except Exception:
                pass
            try:
                ctx.unroute("**/shopping_list**", route_handler)
            except Exception:
                pass

    _try(results, "17", "cross-device cloud cart sync", check_cross_device_cart_sync)
